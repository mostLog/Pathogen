...
namespace L.Application.Services
{
    public class NovelService : INovelService
    {
        ...
        public IList<Novel> GetNovels(NovelSearchInput input)
        {
            ...
            if (input.IsOpenEmail.HasValue)
            {
                whereSql.Append(" and isOpenEmail=@isOpenEmail");
                parameters.Add("isOpenEmail", input.IsOpenEmail.Value);
            }
            parameters.Add("IsCrawlerArticle",input.IsCrawlerArticle);
            using (IDbConnection db=_factory.GetDbInstance())
            {
                return db.QueryList<Novel>(sql+whereSql.ToString(),parameters).ToList();
            }
        }
        ...
        public Article GetArticleById(int id)
        {
            using (IDbConnection db=_factory.GetDbInstance())
            {
                return db.GetEntity<Article>(id);
            }
        }
        ...
        public IList<Article> GetArticles(ArticleSearchInput input)
        {
            ...
            if (input.NovelIds!=null&&input.NovelIds.Count>0)
            {
                whereSql.Append(" and novelId in("+string.Join(",", input.NovelIds) +")");
            }
            ...
            using (IDbConnection db = _factory.GetDbInstance())
            {
                ...
                if (novelIds.Count() > 0)
                {
                    string novelSql = string.Format("select * from t_novel where id in({0})", string.Join(",", novelIds));
                    //获取小说集合
        ...
        /// </summary>
        /// <param name="article"></param>
        public bool UpdateArticel(Article article)
        {
            using (IDbConnection db=_factory.GetDbInstance())
            {
                return db.UpdateEntity(article);
            }
        } 
        #endregion
    }
